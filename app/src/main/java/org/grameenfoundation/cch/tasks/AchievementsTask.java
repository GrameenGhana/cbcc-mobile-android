		...
		//JSONArray jsonArrayCourses;
		 ...
		 try {
			 ...
			 for(int i=0;i<jsonArrayNewCourses.length();i++){
				 ...
				 String course=jsonArrayNewCourses.getJSONObject(i).getString("course_id");
				 String percentcomplete=jsonArrayNewCourses.getJSONObject(i).getString("percompleted");
				 String attempts=jsonArrayNewCourses.getJSONObject(i).getString("attempts");
				 String score=jsonArrayNewCourses.getJSONObject(i).getString("percentscore");
				 String date_taken=jsonArrayNewCourses.getJSONObject(i).getString("datetime");
				 String status=jsonArrayNewCourses.getJSONObject(i).getString("ksa_status");
				 /*if(!score.equals("Not taken")&&Double.valueOf(score)>=85&&Integer.valueOf(percentcomplete)>=85){
					 status="ksa_eligble";
				 }else {
					 status="in_progress";
				 }*/
