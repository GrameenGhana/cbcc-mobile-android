	...
	private boolean DisplayForm()
	{
		...
		try
		{
	        ...
	        for (i=0;i<theForm.fields.size();i++) {
	        	...
	        	if (theForm.fields.elementAt(i).getType().equals("multichoice")) {
	        		theForm.fields.elementAt(i).obj = new XmlGuiMultiChoice(this,(theForm.fields.elementAt(i).isRequired() ? "" : "") + theForm.fields.elementAt(i).getLabel(),theForm.fields.elementAt(i).getOptions());
	        		ll.addView((View) theForm.fields.elementAt(i).obj);
	        	}
	        }
                ...
                public void handleMessage(Message msg) {
                    ...
                    switch (msg.what) {
                        case 0:
		            		...
		            		ad.setButton(DialogInterface.BUTTON_NEGATIVE,"Close", new DialogInterface.OnClickListener(){
		...
		try {
			...
			for (i=0;i<theForm.fields.size();i++) {
				...
				if (theForm.fields.elementAt(i).isRequired()) {
					if(theForm.fields.elementAt(i).getType()=="choice"){
					if (fieldValue == null) {
						good = false;
					} else {
        ...
        public void run() {
            ...
            try { 
                ...
                for (int i=0;i<theForm.fields.size();i++) {
    	        	...
    	        	if(theForm.fields.elementAt(i).getType().equals("multichoice")){
    	        		json.put( theForm.fields.elementAt(i).getName(),  theForm.fields.elementAt(i).getMultiResults());
    	        	}
    	        }
